  ...
  return (
    <div className="min-h-screen bg-app-cream">
              ...
              {totalPages > 1 && (
                <div className="flex-center gap-2 mt-16">
                  {Array.from({length: totalPages}).map((_, i)=>(
                    <button key={i}
                     onClick={()=> {updateFilter("page", String(i + 1)); scrollTo(0,0)}} className={`size-9 rounded-lg text-sm font-medium transition-colors ${page === i + 1 ? "bg-app-green text-white" : "bg-white text-app-text-light hover:bg-app-cream"}`}>
                      {i + 1}
                    </button>
                  ))}
                </div>
      ...
      {mobileFiltersOpen && (
        ...
        <div className="fixed bottom-0 left-0 ring-0 bg-white z-50 rounded-t-2xl max-h-[80vh] overflow-y-auto animate-slide-in-up">
